...
import { MatbutikkFeature, MatbutikkLayerType, MatbutikkProperties, hoverMatbutikkStyleFunction } from './MatbutikkLayer';
...
const MatbutikkAside = () => {
    ...
    const { activeFeatures, setActiveFeatures } = useActiveFeatures<MatbutikkFeature>(
        (l) => l.getClassName() === "MatbutikkerLayer",
      );
    ...
    return (
      <aside className={visibleFeatures?.length ? 'visible' : 'hidden'} style={{ maxHeight: '100vh', overflow: 'auto' }}>
        <div>
          <h2>Matbutikker</h2>
          <ul 
          onMouseLeave={() => setActiveFeatures([])}
          >
