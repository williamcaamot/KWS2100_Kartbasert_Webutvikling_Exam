...
import { MobilityVehicle, MobilityVehiclesResponse, mobilityStyle } from './MobilityFeature';
...
const MOBILITY_QUERY = gql`
    query MobilityQuery($lat: Float!, $lon: Float!, $range: Int!, $count: Int!, $formFactors: [FormFactor]!) {
        vehicles(lat: $lat, lon: $lon, range: $range, count: $count, formFactors: $formFactors) {
            id
            lat
            lon
            pricingPlan {
...
function createFeatureFromMobilityVehicle(vehicle: MobilityVehicle): ol.Feature {
...
const MobilityLayer = () => {
    ...
    const [selectedCity, setSelectedCity] = useState<{ lat: string; lon: string }>({ lat: '59.9139', lon: '10.7522' });
    const [loading, setLoading] = useState(false);

    const fetchMobility = () => {
        setLoading(true);
        request<MobilityVehiclesResponse>('https://api.entur.io/mobility/v2/graphql', MOBILITY_QUERY, {
            ...
            .then((data) => {
                ...
                const features: ol.Feature[] = vehicles.map(createFeatureFromMobilityVehicle);
    ...
    useEffect(() => {
        if (selectedCity.lat === '0' && selectedCity.lon === '0') {
            ...
            FetchUserLocation()
                .then((userLocation) => {
                    setSelectedCity({ lat: userLocation.latitude.toString(), lon: userLocation.longitude.toString() });
                    setLoading(false);
                })
                .catch((error) => {
                    console.error(error);
                ...
                {checked && (
                    <form className='max-w-sm'>
                        <select
                            ...
                            onChange={(e) => setSelectedCity(JSON.parse(e.currentTarget.value))}
                            defaultValue='none'>
                            <option value='none'>Choose a country</option>
                            {mobilityCitiesOptions.map((option) => (
                                <option key={option.value.lat + option.value.lon} value={JSON.stringify(option.value)}>
                                    {option.label}
                                </option>
                            ))}
                        </select>
